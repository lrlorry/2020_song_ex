...
static void write_json(const std::string& path, int n, int nq, int dim, int k, int pq,
                       ...
                       double recall_at_k, const std::vector<std::vector<idx_t>>& gt,
                       const std::vector<std::vector<idx_t>>& results) {
    auto recall = [&](void) {
        float total = 0.0f;
        for (int i = 0; i < nq; ++i) {
            int hit = 0;
            for (int j = 0; j < k && j < (int)results[i].size(); ++j) {
...
int main(int argc, char** argv) {
    ...
    auto t_gt0 = clk::now();
    std::vector<std::vector<idx_t>> gt(args.nq);
    ...
    write_json(args.json_out, args.n, args.nq, dim, args.k, args.pq,
               ms(t0, t1), ms(t2, t3), ms(t_gt0, t_gt1), rec, gt, results);
